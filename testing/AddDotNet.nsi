...
;General
  ...
  ;Name and file
  Name "AddNet"
  OutFile "AddNetSetup.exe"

  ;Default installation folder
  ;InstallDir "$LOCALAPPDATA\Symlix"
  ;InstallDir "$PROGRAMFILES\Symlix"
  
  ;Get installation folder from registry if available - another user may have installed elsewhere
  ;InstallDirRegKey HKCU "Software\Symlix" ""
...
;Section 
  Section $(SEC_DOTNET) SECDOTNET
   DetailPrint " Section$$(SEC_DOTNET) SECDOTNET"
   SectionIn RO
    DetailPrint "about to check if silent"
    IfSilent lbl_IsSilent
    !define DOTNETFILESDIR "Common\Files\MSNET"
    StrCpy $DOTNET_RETURN_CODE "0"
    DetailPrint "About to check language"
!ifdef DOTNET_ONCD_1033
     DetailPrint "Check OSLang = $OSLANGUAGE"
    StrCmp "$OSLANGUAGE" "1033" 0 lbl_Not1033
    SetOutPath "$PLUGINSDIR"
     DetailPrint "SetOutPath $PLUGINSDIR"
...
; Insert Other language blocks here
	...
	DetailPrint "Goto lbl_DownloadRequired"
    ...
    lbl_NoError:
    lbl_IsSilent:
    DetailPrint "passing lbl_IsSilent"
  SectionEnd
...
; Insert other Language Blocks Here
  ...
  DetailPrint "!insertmacro MUI_LANGDLL_DISPLAY"
